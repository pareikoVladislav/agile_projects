...
class ProjectFileListGenericViewTest(TestCase):
    ...
    def test_response_object(self):
        ...
        # Настраиваем атрибуты и методы mock_response
        mock_response.data = {'key': 'value'}
        mock_response.status_code = 200
        mock_response.__str__.return_value = 'Response object'

        # Проверяем, что атрибуты и методы работают как ожидается
        self.assertEqual(mock_response.data, {'key': 'value'})
        self.assertEqual(mock_response.status_code, 200)
        self.assertEqual(str(mock_response), 'Response object')
