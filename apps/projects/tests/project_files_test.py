...
class ProjectFileListGenericViewTest(TestCase):
    field_types = {'id': int, 'file_name': str, 'project': list}
    ...
    @patch(
        'apps.projects.views.project_files_views.ProjectFileListGenericView.get_queryset'
    )
    ...
    def test_get_project_files_with_project_name(self, mock_get_queryset):
        ...
        response = self.client.get(
            '/api/v1/projects/files/?project_name=Test%20Project'
        )
...
class TestCreateProjectFileSerializer(TestCase):
    ...
    def test_create_project_file(self, mock_save_file):
        ...
        context = {"project": project, "file": mock_file}

        data = {"file_name": "mock_file.pdf"}
        ...
        mock_save_file.assert_called_once_with(
            mock_file, 'documents/Mock_Project/mock_file.pdf'
        )

        self.assertEqual(
            project_file.file_path, 'documents/Mock_Project/mock_file.pdf'
        )
